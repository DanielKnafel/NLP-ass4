...
def get_label(t,sentences,RE = 'Live_In'):
    ...
    return 0
RE_sentences = []
def get_dataset_from_file_and_tuples(file, tuples,RE = 'Live_In',marker=False):
    global RE_sentences
    data = []
    ...
    for i,line in enumerate(lines):
        ...
        if line[0] in tuples.keys():
            ...
            for t in tuples[line[0]]:
                ...
                data.append({'id':line[0],'idx': idx, 'sent1': sent1, 'sent2': " ".join(t[0:3]), 'label': get_label(t,sent_lines)})
                idx += 1
        last_line = line[0]
    return data
...
get_True_False_RE_from_files('../data/Corpus.TRAIN.txt','../data/TRAIN.annotations')
...
json_to_file(marked_train_data, '../data/TRAIN.marked.json')
json_to_file(marked_dev_data, '../data/DEV.marked.json')
